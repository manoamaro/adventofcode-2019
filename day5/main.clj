...
(defn compute [operations]
  (let [operations-size (count operations)]
    (loop [result operations pos 0]
      (let [opcode (nth result pos)]
        (cond
          (or (= opcode 99) (>= pos operations-size)) result
          (= opcode 1) (let [idx1 (nth result (+ pos 1))
          ...
          (= opcode 2) (let [idx1 (nth result (+ pos 1))
                             idx2 (nth result (+ pos 2))
                             idx3 (nth result (+ pos 3))
                             v1 (nth result idx1)
                             v2 (nth result idx2)
          ...
          (= opcode 3) (let [idx1 (nth result (inc pos))] (recur (replace-at result idx1 input) (+ pos 2)))
          (= opcode 4) (let [idx1 (nth result (inc pos))
          ...
          :else (let [opcode-str (str opcode)
                  ...
                  (cond
                    (= new-opcode 1) (let [idx1 (nth result (+ pos 1))
                                           idx2 (nth result (+ pos 2))
                                           idx3 (nth result (+ pos 3))
                    ...
                    (= new-opcode 2) (let [idx1 (nth result (+ pos 1))
                                           idx2 (nth result (+ pos 2))
                                           idx3 (nth result (+ pos 3))
                                           v1 (if (= arg-type-1 \1) idx1 (nth result idx1))
                                           v2 (if (= arg-type-2 \1) idx2 (nth result idx2))
